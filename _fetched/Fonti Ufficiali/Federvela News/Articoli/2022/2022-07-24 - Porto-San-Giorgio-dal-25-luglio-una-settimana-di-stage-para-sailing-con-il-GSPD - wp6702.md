# Porto San Giorgio: dal 25 luglio una settimana di stage para sailing con il GSPD

Stato fonte: **ufficiale FIV / Federvela**.
Uso nel vault: memoria ufficiale, contesto istituzionale, risultati e traccia editoriale. Per classifiche sportive usare comunque il dato finale quando disponibile.

## Fonte

- Data Federvela: 24 Luglio 2022
- Link: https://federvela.it/2022/07/24/porto-san-giorgio-dal-25-luglio-una-settimana-di-stage-para-sailing-con-il-gspd/
- Categorie: parasailing
- Archiviato: 2026-08-23

## Perche' tenerla

- Filoni: olimpica, parasailing/inclusione, istituzionale, zone/circoli
- Valore: fonte federale primaria per ricostruire decisioni, eventi, risultati, territori, progetti e comunicazione FIV.

## Sintesi operativa

- Temi operativi: olimpica, parasailing/inclusione, istituzionale, zone/circoli.
- Angoli memoria: inclusione, parasailing e valore istituzionale, atto, accordo, governance o policy federale, territori, zone e circoli, classi olimpiche o alto livello.
- Classi/discipline citate: Hansa, 2.4, Para Sailing.
- Nomi, circoli o luoghi da valutare: Porto San Giorgio, GSPD Porto San Giorgio, GSPD Data, Para Sailing Sarà, Circolo ASD Liberi, Vento di Porto, Vento di Porto San, Giorgio, Federazione Italiana Vela, Gruppo Sportivo Paralimpico, Difesa, GSPD, Hansa, Fabio Barbieri e Filippo.
- Numeri/date utili: 25, 24, 2022, 303, 2, 2028.
- Testo fonte ufficiale disponibile online: usare il link per verifica puntuale, senza duplicare integralmente l'articolo nel vault.

## Collegamenti utili

- [[Istituzionale FIV/00-Overview|Istituzionale FIV]]
- [[World Sailing - Mappa Atleti e Classifiche]]
- [[comunicati stampa sportivi/_Indice-Atleti-Circoli|Atleti e circoli]]
- [[Modelli/Modello-News-Istituzionale|Modello news istituzionale]]
- [[Modelli/Modello-News-Evento|Modello news evento]]

## Uso editoriale

- Se riguarda risultati: usarla come fonte ufficiale FIV e collegare eventuale classifica finale World Sailing quando presente.
- Se riguarda governance, accordi o progetti: collegarla alla memoria istituzionale.
- Se riguarda zone/circoli: collegarla a territorio, circoli e comunicati sportivi.
